...
export default function Join() {
  const [submittedProvider, setSubmittedProvider] = React.useState(false);
  const [loadingProvider, setLoadingProvider] = React.useState(false);
  const [submittedRenter, setSubmittedRenter] = React.useState(false);
  const [loadingRenter, setLoadingRenter] = React.useState(false);

  const handleSubmitProvider = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (submittedProvider || loadingProvider) return;
    setLoadingProvider(true);
    window.setTimeout(() => {
      setLoadingProvider(false);
      setSubmittedProvider(true);
    }, 1500);
  };

  const handleSubmitRenter = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (submittedRenter || loadingRenter) return;
    setLoadingRenter(true);
    window.setTimeout(() => {
      setLoadingRenter(false);
      setSubmittedRenter(true);
    }, 1500);
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground pl-[260px]">
      <div className="mx-auto max-w-6xl px-8 py-10">
        ...
        <div className="mt-8 grid gap-10 md:grid-cols-2">
          <section className="glass-panel p-6 flex flex-col">
            ...
            <form className="mt-4 grid gap-3 flex-1" onSubmit={handleSubmitProvider}>
              ...
              <button
                type="submit"
                disabled={submittedProvider || loadingProvider}
              ...
              >
                <span className="block text-sm font-semibold tracking-[0.18em] uppercase">
                  {submittedProvider ? "SUBMITTED" : loadingProvider ? "VALIDATING CREDENTIALS..." : "CONFIRM EXPRESSION OF INTEREST"}
                </span>
          ...
          <section className="glass-panel p-6 flex flex-col">
            ...
            <form className="mt-4 grid gap-3 flex-1" onSubmit={handleSubmitRenter}>
              ...
              <button
                type="submit"
                disabled={submittedRenter || loadingRenter}
              ...
              >
                <span className="block text-sm font-semibold tracking-[0.18em] uppercase">
                  {submittedRenter ? "SUBMITTED" : loadingRenter ? "VALIDATING CREDENTIALS..." : "CONFIRM EXPRESSION OF INTEREST"}
                </span>
                <span className="mt-1 block text-[11px] text-muted-foreground">
                  Secure Enrollment into XGPU Early Access Pool
                </span>
        ...
        {submittedProvider || submittedRenter ? (
